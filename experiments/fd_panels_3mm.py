...
def solve_fd(mask_c, k2_c, q_c, spacing_mm, erode=1):
    ...
    coords = np.argwhere(D); nD = coords.shape[0]; idx[D] = np.arange(nD)
    cx, cy, cz = coords.T
    ...
    qc = np.full(mask_c.shape, np.nan, np.complex128); qc[D] = x; qc[Sb] = q_c[Sb]
    return qc
...
fri = RegularGridInterpolator(axes, qc.real, method="linear", bounds_error=False, fill_value=np.nan)
fii = RegularGridInterpolator(axes, qc.imag, method="linear", bounds_error=False, fill_value=np.nan)
nx, ny = mask.shape[:2]
...
meas_sl = q[:, :, Z]
CNAN = complex(np.nan, np.nan)  # complex NaN: masks BOTH Re and Im (nan+0j would leave Im=0)
...
rr = np.where(mslice.any(1))[0]; cc = np.where(mslice.any(0))[0]
PAD = 5
...
def vlim(part):
    ...
    v = np.nanpercentile(np.abs(getattr(meas_d, part)), 99); return -v, v
...
for r, (part, cmap) in enumerate(rows):
    ...
    fig.savefig(f"{OUT}/fdm_panel_{part}.png", dpi=100, facecolor="white"); plt.close(fig)

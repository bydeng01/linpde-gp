...
)  # <repo>/experiments/.. == repo root
ROOT = f"{REPO}/data/brain_experiment_data/mre_udel"
OUTDIR = os.environ.get("OUTDIR", f"{REPO}/experiments/helmholtz_brain_outputs")
ORIG_PDF = os.environ.get(
    "ORIG_PDF", f"{OUTDIR}/helmholtz_brain_U01_UDEL_0003_01_70Hz_compx_curl_main.pdf"
)
...
def solve_fd(mask_c, k2_c, q_c, spacing_mm, erode=1):
    ...
    if np.linalg.norm(A @ x - b) > 1e-5 * np.linalg.norm(
        b
    ):  # direct fallback if iterative struggles
...
MODE = os.environ.get(
    "FD_DISPLAY", "full"
)  # "full" (1.5 mm, == the Pearson-0.855 baseline) or "3mm"
if MODE == "3mm":
    s = 2
    qc = solve_fd(
        mask[::s, ::s, ::s], k2[::s, ::s, ::s], q[::s, ::s, ::s], zooms * s, erode=1
    )
    axc = tuple(np.arange(n) for n in qc.shape)
    fri = RegularGridInterpolator(
        axc, qc.real, method="linear", bounds_error=False, fill_value=np.nan
    )
    fii = RegularGridInterpolator(
        axc, qc.imag, method="linear", bounds_error=False, fill_value=np.nan
    )
    nx, ny = mask.shape[:2]
    ...
    print(
        f"[fdm] full-res corr(|qFD|,|qmeas|) on E = "
...
r0, r1 = np.where(sil.any(1))[0][[0, -1]]
c0, c1 = np.where(sil.any(0))[0][[0, -1]]  # tight bbox
...
def vlim(part):
    if part == "abs":
        return 0.0, float(np.nanpercentile(np.abs(meas_d), 99))
    v = float(np.nanpercentile(np.abs(getattr(meas_d, part)), 99))
    return -v, v
    ...
    for _l in _b.get("lines", []):
        for _sp in _l["spans"]:
            if _sp["text"].strip() == "Measured":
                hdr_top = _sp["bbox"][1] * zoom
                hdr_size = _sp["size"] * zoom
arr = np.asarray(img)
nw = ~np.all(arr > 245, axis=2)
H, W = nw.shape
...
pitch = px0 - mx0
print(
    f"[geometry] brain {brain_w}x{brain_h}  meas_x0={mx0} pred_x0={px0} "
...
panels = {
    p: render_brain(p, cm)
    for p, cm in [("real", "RdBu_r"), ("imag", "RdBu_r"), ("abs", "viridis")]
}
...
local_x = mx0 - (
    px0 - pitch
)  # brain offset within its column (== 0: column-left, like Measured)
...
font = ImageFont.truetype(
    fm.findfont(fm.FontProperties(weight="bold")), int(round(hdr_size))
)
tb = draw.textbbox((0, 0), "FDM", font=font)
draw.text(
    (local_x + (brain_w - (tb[2] - tb[0])) / 2, hdr_top - tb[1]),
    "FDM",
    fill=(0, 0, 0),
    font=font,
)
...
fsl = np.asarray(panels["abs"])
fnw = ~np.all(fsl > 245, axis=2)
om = nw[row_tops[2] : row_tops[2] + brain_h, mx0 : mx0 + brain_w]
hh = min(fnw.shape[0], om.shape[0])
ww = min(fnw.shape[1], om.shape[1])
iou = (fnw[:hh, :ww] & om[:hh, :ww]).sum() / max(
    1, (fnw[:hh, :ww] | om[:hh, :ww]).sum()
)
print(
    f"[verify] FDM vs Measured |.| silhouette IoU = {iou:.3f}  (1.0 = identical size/outline)"
)

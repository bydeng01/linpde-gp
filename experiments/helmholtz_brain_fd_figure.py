...
for r, (part, lab, cmap) in enumerate(rows):
    ...
    pred = chan(qFD, part)
    if part == "abs":
        vmax = np.nanpercentile(meas, 99)
        vmin = 0.0
    else:
        vmax = np.nanpercentile(np.abs(meas), 99)
        vmin = -vmax
...
for ext in ("pdf", "png"):
    fig.savefig(
        f"{OUT}/fd_vs_measured_0003_70Hz_compx.{ext}", dpi=200, bbox_inches="tight"
    )

...
from scipy.sparse.linalg import splu, spilu, bicgstab, LinearOperator
...
def main():
    ...
    print(f"[check] measured |r|/|k^2 q| median on E = {np.median(rel[E]):.3f}  (paper 3.11)")
    ...
    diag = (-2*(1/dx**2 + 1/dy**2 + 1/dz**2) + k2[cx,cy,cz]).astype(np.complex128)
    rows = [np.arange(nD)]; cols = [np.arange(nD)]; vals = [diag]
    ...
    print(f"BiCGStab info={info} iters={it['n']} in {time.time()-t:.1f}s  ||Ax-b||/||b||={relres:.2e}", flush=True)
    if relres > 1e-5:  # fallback to direct if iterative struggled
        print("iterative residual high; falling back to splu...", flush=True)
        x = splu(A).solve(b)
        print(f"[check] direct ||Ax-b||/||b|| = {np.linalg.norm(A@x-b)/np.linalg.norm(b):.2e}", flush=True)

    qFD = np.full(mask.shape, np.nan, np.complex128)
    qFD[D] = x; qFD[S] = q[S]
    np.savez(f"{OUT}/fd_brain_0003_70Hz_compx.npz",
             qFD=qFD, q=q, mask=mask, D=D, S=S, E=E, k2=k2, zooms=zooms)

    # --- stability + correlation on E
    magFD = np.abs(qFD[E]); magM = np.abs(q[E])
    print(f"[stability] max|qFD|/max|qmeas| on interior = {np.nanmax(np.abs(qFD[D]))/np.max(np.abs(q[D])):.2f}")
    print(f"[result] corr(|qFD|,|qmeas|) on E = {np.corrcoef(magFD,magM)[0,1]:.3f}  (n={E.sum()})")
    magFD_D = np.abs(qFD[D]); magM_D = np.abs(q[D])
    print(f"[result] corr(|qFD|,|qmeas|) on D = {np.corrcoef(magFD_D,magM_D)[0,1]:.3f}  (n={D.sum()})")

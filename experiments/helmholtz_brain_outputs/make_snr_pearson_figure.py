...
x70 = np.array([p[0] for p in hz70])
y70 = np.array([p[1] for p in hz70])
...
for sx, sy, lab in hz70:
    ...
    ax.annotate(
        lab, (sx, sy), (sx + dx, sy + dy), fontsize=6, color="#1f1f1f", ha=ha, va=va
    )
...
for (ax0, ay0), (ax1, ay1) in zip(traj[:-1], traj[1:]):
    ax.annotate(
        ...
        xytext=(ax0, ay0),
        arrowprops=dict(
            arrowstyle="-|>", color=GREY, lw=0.8, alpha=0.55, shrinkA=6, shrinkB=6
        ),
    )
# low-freq points
for (sx, sy, lab), c, mk in [(lowf[0], COL50, "D"), (lowf[1], COL30, "s")]:
    ax.scatter(
        sx, sy, s=40, color=c, marker=mk, edgecolor="white", linewidth=0.6, zorder=6
    )
    ax.annotate(
        lab,
        (sx, sy),
...
print(
    f"70Hz trend: r={r:.3f} p={p:.4f} slope={lr.slope:.4f} intercept={lr.intercept:.4f}"
)

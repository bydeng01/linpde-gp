...
def case_matern_identity_weighted_laplacian_multid(
    input_shape: ShapeType, nu: float
) -> CovarianceFunctionDiffOpTestCase:
    rng = np.random.default_rng(2025_06_01)

    weights = 2.0 * rng.standard_normal(size=input_shape)
    lengthscales = 0.5 + rng.uniform(size=input_shape)

    return CovarianceFunctionDiffOpTestCase(
        k=covfuncs.Matern(input_shape, nu=nu, lengthscales=lengthscales),
        L0=None,
        L1=diffops.WeightedLaplacian(weights),
        expected_type=covfuncs_diffops.HalfIntegerMatern_Identity_WeightedLaplacian,
    )
...
def case_matern_weighted_laplacian_weighted_laplacian_multid(
    input_shape: ShapeType, nu: float
) -> CovarianceFunctionDiffOpTestCase:
    rng = np.random.default_rng(2025_06_02)
    ...
    return CovarianceFunctionDiffOpTestCase(
        ...
        L0=diffops.WeightedLaplacian(weights0),
        L1=diffops.WeightedLaplacian(weights1),
        expected_type=covfuncs_diffops.HalfIntegerMatern_WeightedLaplacian_WeightedLaplacian,  # pylint: disable=line-too-long
    )

...
def compute_pde_residual_finite_diff(u_func, k_squared, rhs_func, X_test, h=1e-5):
    ...
    u_plus = u_func(X_test + h) if callable(u_func) else u_func.mean(X_test + h).flatten()
    u_minus = u_func(X_test - h) if callable(u_func) else u_func.mean(X_test - h).flatten()
...
def test_compare_solutions(bvp):
    ...
    u_prior = pn.randprocs.GaussianProcess(
        mean=linpde_gp.functions.Zero(input_shape=bvp.domain.shape),  # Match domain shape
        cov=output_scale**2
        * linpde_gp.randprocs.covfuncs.Matern(bvp.domain.shape, nu=2.5, lengthscales=lengthscale_x),
    )
...
def test_helmholtz_with_zero_boundary_conditions():
    ...
    bvp = linpde_gp.problems.pde.HelmholtzEquationDirichletProblem(
        ...
        G_real=1.0,
        G_imag=0.0,
        boundary_values=linpde_gp.functions.Zero(
            spatial_domain.shape, output_shape=()
        ),
    )
...
def test_helmholtz_with_analytical_solution():
    ...
    u_prior = pn.randprocs.GaussianProcess(
        mean=linpde_gp.functions.Zero(input_shape=bvp.domain.shape),
        cov=output_scale**2 * linpde_gp.randprocs.covfuncs.Matern(
    ...
    u_bc = u_prior.condition_on_observations(
        Y_bc, X=X_bc, 
        b=pn.randvars.Normal(np.zeros((2, 1)), 1e-8 * np.eye(2))
    )
...
def test_helmholtz_complex_wave_number():
    ...
    residual = compute_pde_residual_finite_diff(
        bvp.solution, bvp.pde.k_squared, lambda x: 0.0, 
...
def test_pde_residual_validation():
    ...
    u_bc = u_prior.condition_on_observations(
        Y_bc, X=X_bc,
        b=pn.randvars.Normal(np.zeros((2, 1)), 1e-10 * np.eye(2))
    )
    ...
    X_test_interior = spatial_domain.uniform_grid(15, inset=0.3)  # Even fewer test points, larger inset
    residual_gp = compute_pde_residual_finite_diff(
        lambda x: u_gp.mean(x.reshape(-1, 1)).flatten(),
        k_squared_gp, lambda x: 0.0, X_test_interior
    )
...
@pytest.mark.skip(reason="probnum.randvars.Normal doesn't support 3D tensors for vector-valued GPs with multi-point observations")
...
def test_constant_function_G_matches_scalar_path_real():
    ...
    pde_scalar = HelmholtzEquation(
        domain=domain, rho=rho, omega=omega,
        G_real=G_real_val, G_imag=0.0,
    )
    pde_func = HelmholtzEquation(
        domain=domain, rho=rho, omega=omega,
        G_real=Constant(domain.shape, value=G_real_val), G_imag=0.0,
    )
    ...
    f = JaxLambdaFunction(
        lambda x: jnp.sin(x),
        input_shape=domain.shape, output_shape=(), vectorize=True,
    )
...
def test_constant_function_G_matches_scalar_path_real2():
    ...
    pde_scalar = HelmholtzEquation(
        domain=domain, rho=rho, omega=omega,
        G_real=G_real_val, G_imag=G_imag_val,
        complex_representation="real2",
    )
    pde_func = HelmholtzEquation(
        domain=domain, rho=rho, omega=omega,
    ...
    f = JaxLambdaFunction(
        ...
        input_shape=domain.shape, output_shape=(2,), vectorize=True,
    )
...
def test_variable_G_with_real2_representation_constructs_cleanly():
    ...
    assert isinstance(
        pde.diffop, linpde_gp.linfuncops.diffops.HelmholtzReal2Operator
    )

...
class BlockMatrix2x2(pn.linops.LinearOperator):
    ...
    def __init__(
        ...
        if B is None and C is None:
            ...
            # MODIFICATION: For complex matrices, skip the positive definite assertion
            # as it may not be properly set yet
            if not np.issubdtype(self._A.dtype, np.complexfloating):
                assert self._A.is_positive_definite
            # END MODIFICATION
            if C is None:

...
class JaxLambdaCovarianceFunction(JaxCovarianceFunction):
    ...
    def _evaluate(self, x0: np.ndarray, x1: np.ndarray) -> np.ndarray:
        # When `x1` is None, probnum expects pointwise covariances
        # k(x, x) with shape `batch_shape + output_shape_0 + output_shape_1`.
        # Some callables passed in here (e.g. jitted expressions) return a
        # full pairwise matrix instead, so we extract the diagonal in that case.
        if x1 is None:
            x1 = x0
            ...
            doubled_batch_shape = batch_shape + batch_shape
            if k_x0_x0.shape[: len(doubled_batch_shape)] == doubled_batch_shape:
                # Index along matching batch axes to pull out the pointwise
                # diagonal instead of the full pairwise matrix.
                if batch_shape:
                    idx = np.indices(batch_shape)
                    k_x0_x0 = k_x0_x0[tuple(idx) + tuple(idx)]
                else:
                    k_x0_x0 = k_x0_x0.reshape(expected_shape)

            if k_x0_x0.shape != expected_shape:
                k_x0_x0 = np.reshape(k_x0_x0, expected_shape)
    ...
    def _evaluate_jax(self, x0: jnp.ndarray, x1: Optional[jnp.ndarray]) -> jnp.ndarray:
        if x1 is None:
            ...
            if k_x0_x0.shape[: len(doubled_batch_shape)] == doubled_batch_shape:
                if batch_shape:
                    idx = jnp.indices(batch_shape)
                    k_x0_x0 = k_x0_x0[tuple(idx) + tuple(idx)]
                else:
                    k_x0_x0 = jnp.reshape(k_x0_x0, expected_shape)

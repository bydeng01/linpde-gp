...
class TensorProduct_LinDiffOp_LinDiffOp(JaxSumCovarianceFunction[TensorProduct]):
    ...
    def linop(
        ...
        if isinstance(x0, TensorProductGrid) and (
            ...
            # Build Kronecker products for each summand and then sum them
            # We can't use _compute_res with res_zero_value=0 for linops because
            # adding 0 to a non-square linear operator fails (Scaling must be square)
            L0kL1_evals = np.empty(self.k.input_shape, dtype=dict)
            for idx, _ in np.ndenumerate(L0kL1_evals):
                L0kL1_evals[idx] = {}
                ...
                for L1_multi_index, L1_coeff in L1_coeffs.items():
                    ...
                    for domain_idx, _ in np.ndenumerate(L0kL1_evals):
                        ...
                        if orders_idx not in L0kL1_evals[domain_idx]:
                            ...
                            L0kL1_evals[domain_idx][orders_idx] = linop
                        factors.append(L0kL1_evals[domain_idx][orders_idx])

                    kronecker_product = functools.reduce(pn.linops.Kronecker, factors)
                    summands.append(L0_coeff * L1_coeff * kronecker_product)

            return functools.reduce(operator.add, summands)

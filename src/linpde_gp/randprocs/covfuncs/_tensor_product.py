...
class TensorProduct(
    ...
    def linop(
        ...
        if isinstance(x0, TensorProductGrid) and (
            ...
            # Set symmetry flags on individual factors when x1 is None or same as x0
            # (i.e., when computing Gram matrices)
            if x1 is None or x1 is x0:
                for factor in kronecker_factors:
                    factor.is_symmetric = True
                    factor.is_positive_definite = True

...
class ConditionalGaussianProcess(pn.randprocs.GaussianProcess):
    ...
    class CovarianceFunction(JaxCovarianceFunction):
        ...
        def _evaluate(self, x0: np.ndarray, x1: np.ndarray | None) -> np.ndarray:
            ...
            randvar_size = self._kLas.randvar_size
            output_size = int(np.prod(self._kLas.randproc_output_shape, dtype=int))

            def _flatten_kLa(kLa_eval: np.ndarray) -> np.ndarray:
                batch_ndim = (
                    kLa_eval.ndim
                    - self._kLas.randproc_output_ndim
                    - self._kLas.randvar_ndim
                )
                batch_shape = kLa_eval.shape[:batch_ndim]
                return np.reshape(
                    kLa_eval,
                    batch_shape + (output_size, randvar_size),
                    order="C",
                )

            kLas_x0_mat = _flatten_kLa(kLas_x0)
            kLas_x1_mat = _flatten_kLa(kLas_x1)

            solved = self._gram_matrix.solve(np.swapaxes(kLas_x1_mat, -2, -1))
            cov_update = kLas_x0_mat @ solved

            cov_update = np.reshape(cov_update, k_xx.shape, order="C")
            ...
            if x1 is None or (
                ...
                cov_reshaped = np.reshape(
                    cov,
                    (-1, self.output_size_0, self.output_size_1),
                    order="C",
                )
                ...
                cov = np.reshape(cov_reshaped, cov.shape, order="C")
        ...
        def _evaluate_jax(self, x0: jnp.ndarray, x1: jnp.ndarray | None) -> jnp.ndarray:
            ...
            def _flatten_kLa(kLa_eval: jnp.ndarray) -> jnp.ndarray:
                batch_ndim = (
                    kLa_eval.ndim
                    - self._kLas.randproc_output_ndim
                    - self._kLas.randvar_ndim
            ...
            solved = self._gram_matrix.solve(jnp.swapaxes(kLas_x1_mat, -2, -1))
            cov_update = kLas_x0_mat @ solved

            cov_update = jnp.reshape(cov_update, k_xx.shape, order="C")
        ...
        def matrix(self, x0: np.ndarray, x1: np.ndarray | None = None) -> np.ndarray:
            ...
            asymmetries or slightly negative eigenvalues. We project the Gram matrix
            back to the PSD cone in the square case to make downstream sampling robust.
            """
    ...
    def _preprocess_observations(
        ...
        *,
        ...
        if (
            ...
            if output_ndim > 0:
                batch_ndim = len(eval_functl.X_batch_shape)
                natural_shape = (
                    eval_functl.X_batch_shape + eval_functl.input_codomain_shape
                )

                if Y.shape == natural_shape:
                    Y = np.moveaxis(
                        Y,
                        np.arange(batch_ndim, batch_ndim + output_ndim),
                        np.arange(output_ndim),
                    )
        # Allow users to omit trailing singleton output dims (common for scalar-valued
        # observations shaped like the input batch).
        if (
            len(L.output_shape) > 0
            and L.output_shape[-1] == 1
            and Y.shape == L.output_shape[:-1]
        ):

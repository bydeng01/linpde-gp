...
def _register_prior_predictive_crosscov_handler():
    ...
    def _(
        self,
        pv_crosscov: ConditionalGaussianProcess.PriorPredictiveCrossCovariance,
        /,
    ):
        # Apply the operator to each constituent cross-covariance
        return ConditionalGaussianProcess.PriorPredictiveCrossCovariance(
            tuple(self(kLa) for kLa in pv_crosscov)
        )


_register_prior_predictive_crosscov_handler()
